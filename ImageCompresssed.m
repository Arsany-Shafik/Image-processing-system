function varargout = ImageCompresssed(varargin)

% IMAGECOMPRESSSED MATLAB code for ImageCompresssed.fig
%      IMAGECOMPRESSSED, by itself, creates a new IMAGECOMPRESSSED or raises the existing
%      singleton*.
%
%      H = IMAGECOMPRESSSED returns the handle to a new IMAGECOMPRESSSED or the handle to
%      the existing singleton*.
%
%      IMAGECOMPRESSSED('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in IMAGECOMPRESSSED.M with the given input arguments.
%
%      IMAGECOMPRESSSED('Property','Value',...) creates a new IMAGECOMPRESSSED or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ImageCompresssed_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ImageCompresssed_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ImageCompresssed

% Last Modified by GUIDE v2.5 29-Dec-2022 13:10:39

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @ImageCompresssed_OpeningFcn, ...
                   'gui_OutputFcn',  @ImageCompresssed_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before ImageCompresssed is made visible.
function ImageCompresssed_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ImageCompresssed (see VARARGIN)

% Choose default command line output for ImageCompresssed
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes ImageCompresssed wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = ImageCompresssed_OutputFcn(hObject, eventdata, handles) 
...
% handles    structure with handles and user data (see GUIDATA)
global I;
[fileName,path]=uigetfile({'.tif'},'Select an image');
if fileName==0
    disp("User Caneled");
else
...
M=8;
I_dim=size(hq);              % Finding the dimensions of the image file.
    ...
    for b=1:I_dim(2)/M
        for k=1:N
            for l=1:M
                ...
                for i=1:N
                    for j=1:M
                        prod=prod+double(hq(N*(a-1)+i,M*(b-1)+j))*cos(pi*(k-1)*(2*i-1)/(2*N))*cos(pi*(l-1)*(2*j-1)/(2*M));
                    end
                end
                if k==1
                    prod=prod*sqrt(1/N);
...
diff=im2double(hq)*255-I_rec;
diff=diff./max(max(diff));
diff=im2uint16(diff);
I_rec=I_rec./max(max(I_rec));
I_rec=im2uint16(I_rec);

axes(handles.axes3);
imshow(I_rec),title('Image Reconstructed');
axes(handles.axes4);
imhist(I_rec),title('histogram Reconstructed');

axes(handles.axes5)
imshow(diff),title('Difference');
axes(handles.axes6)
imhist(diff),title('histogram Difference');
